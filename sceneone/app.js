// ─── TURNSTILE CAPTCHA ────────────────────────────────────────────────────
// Site key is the PUBLIC key from Cloudflare Turnstile — safe to be in HTML.
const TURNSTILE_SITE_KEY = '0x4AAAAAADrnusj8lT1xi7OE';
const _tsTokens = {};

function _initTurnstile() {
  if (!window.turnstile) return setTimeout(_initTurnstile, 200);
  const widgets = [
    { id: 'turnstile-signup',     key: 'signup'      },
    { id: 'turnstile-login',      key: 'login'       },
    { id: 'turnstile-exec-signup',key: 'exec-signup' },
    { id: 'turnstile-exec-signin',key: 'exec-signin' },
  ];
  widgets.forEach(({ id, key }) => {
    const el = document.getElementById(id);
    if (!el || el.dataset.tsReady) return;
    el.dataset.tsReady = '1';
    window.turnstile.render(el, {
      sitekey: TURNSTILE_SITE_KEY,
      theme: 'dark',
      callback: (token) => { _tsTokens[key] = token; },
      'expired-callback': () => { delete _tsTokens[key]; },
      'error-callback': () => { delete _tsTokens[key]; },
    });
  });
}
// Init after Turnstile script loads
...
(function keepWarm() {
  const ping = () => fetch(EDGE_FN_URL, { method: 'GET' }).catch(() => {});
  ping();
...
function genStars(){
  const c=document.getElementById('stars'); if(!c)return; c.innerHTML='';
  for(let i=0;i<200;i++){
    const s=document.createElement('div'); s.className='star';
    const sz=Math.random()*2.4+0.4;
    s.style.cssText=`left:${Math.random()*100}%;top:${Math.random()*100}%;width:${sz}px;height:${sz}px;--dur:${2+Math.random()*4}s;--delay:${Math.random()*4}s;--mop:${.1+Math.random()*.3};`;
    c.appendChild(s);
  }
}
...
// ── Star Wars ──
function runStarWars(){
  document.getElementById('loader-starwars').style.display='block';
  ...
  genStars();
  setTimeout(()=>document.getElementById('ep-logo').classList.add('go'), 500);
  setTimeout(()=>{
    document.getElementById('crawl-text').classList.add('go');
    document.getElementById('live-stream').classList.add('show');
    runLiveStream();
  }, 3500);
  runProgressBar(readingPhrases, 18000, '"Scene Set. Your script report is ready."');
}
...
function runLiveStream(){
  ...
  let idx = 0;
  function next(){
    if(idx > 0) items[idx-1].classList.remove('active'), items[idx-1].classList.add('done');
    if(idx < items.length){ items[idx].classList.add('active'); idx++; setTimeout(next, 1800+Math.random()*800); }
  }
  next();
}
...
function runCountdown(){
  document.getElementById('loader-starwars').style.display='none';
...
function reanalyze(edId,flId){
  const btn=document.querySelector(`#${edId} .re-btn`);
  btn.textContent='Analyzing...'; btn.style.opacity='.6';
  setTimeout(()=>{
    btn.textContent='Re-Analyze This Scene'; btn.style.opacity='1';
    document.getElementById(flId).classList.add('show');
    document.getElementById(edId).classList.remove('open');
  },1800);
}
...
async function startOnboarding() {
  ...
  const { data, error } = await supabaseClient.auth.signUp({
    email,
    password,
    options: { data: { full_name: name }, captchaToken: _tsTokens['signup'] }
  });
  ...
  if (error) {
    ...
    const displayMsg = isEmailTaken
      ? 'An account with that email already exists. <span style="text-decoration:underline;cursor:pointer;" onclick="goTo(\'signin\')">Sign in instead?</span>'
...
window.addEventListener('load',()=>{
  ...
  if(writerBtn) writerBtn.addEventListener('click', () => goTo('welcome'));
  if(execBtn)   execBtn.addEventListener('click',   () => goTo('exec-signup'));
...
async function loadDashboard() {
  ...
  try {
    ...
    if (meta) meta.textContent = `${rows.length} script${rows.length!==1?'s':''} available · Sorted by ${_dashSort}`;
...
async function requestScriptAccess(id, title) {
  ...
  const { data: profile } = await supabaseClient.from('profiles').select('role,verified').eq('id',_currentUser.id).single();
  if (!profile || profile.role !== 'exec' || !profile.verified) { goTo('exec-signup'); return; }
  // Log the request (Phase 3: notify writer via Resend)
  const btn = event?.target;
  if (btn) { btn.textContent = '✓ Requested'; btn.disabled = true; btn.style.background='rgba(76,175,125,0.2)'; btn.style.color='var(--green)'; btn.style.borderColor='rgba(76,175,125,0.3)'; }
}
...
async function handleExecSignIn() {
  ...
  const { data, error } = await supabaseClient.auth.signInWithPassword({ email, password, options: { captchaToken: _tsTokens['exec-signin'] } });
  if (error || !data.user) {
...
async function handleExecSignup() {
  ...
  const { data, error } = await supabaseClient.auth.signUp({
    ...
    password,
    options: {
      captchaToken: _tsTokens['exec-signup'],
      data: {
        full_name: name,
        role: 'exec',
        company,
...
// ─── CONTACT PREFERENCE ───
function setContactPref(type, btn) {
  document.querySelectorAll('.contact-toggle button').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
...
async function handleLogin() {
  ...
  const { data, error } = await supabaseClient.auth.signInWithPassword({ email, password: pass, options: { captchaToken: _tsTokens['login'] } });
...
function updateOverallScore() {
  // Recalculate overall from current scores
  const scores = [82, 71, 91, 68, 80]; // base scores — update as CC applied
  const overallEl = document.querySelector('.sidebar-score-big');
  const topStatEl = document.querySelector('.top-stat-val');
  if (!overallEl) return;
  const avg = Math.round(scores.reduce((a,b)=>a+b,0)/scores.length);
  // Burn the overall too
  overallEl.style.transition = 'all .3s';
  overallEl.style.filter = 'brightness(1.5)';
  setTimeout(() => {
    overallEl.textContent = avg + 1; // slight bump for drama
...
function toggleListing(checkbox) {
  ...
  if (checkbox.checked) {
    ...
    if (_currentSubId) {
      supabaseClient.from('submissions').update({ public_listing: false }).eq('id', _currentSubId).then(() => {});
    }
  }
}
...
function confirmWriterListing() {
  ...
  // Write to Supabase
  if (_currentSubId) {
    supabaseClient.from('submissions').update({ public_listing: true }).eq('id', _currentSubId).then(() => {});
  }
...
function confirmTitle() {
  ...
  document.getElementById('crawl-title').textContent = val;
  document.getElementById('rep-title').textContent = val + ' — Draft 1';
...
function getNextLoader() {
  // Star Wars crawl (0) retired — rotate between Film Countdown (1) and Clapperboard (2)
  let next;
  do { next = 1 + Math.floor(Math.random() * 2); } while (next === _lastLoader);
  _lastLoader = next;
  return next;
}
...
async function _runAnalysis() {
  ...
  body: { script_text: scriptText, title, user_email: _currentUser?.email || null, script_type: _scriptType, is_sample: !!window._isSampleRun }
    ...
    _onApiDone(data);
    // Don't re-save / re-count an identical re-upload served from cache —
    // the submission already exists from the first analysis of this draft.
    if (!data.cached) _saveSubmission(title, data);
...
function _showLimitReached(msg) {
  ...
  const detail = msg || "You’ve used all of this month’s script analyses on your current plan.";
